...
### 단기 계획
- [ ] 실제 공공 API 연동 (API 엔드포인트 URL 설정 필요)
- [ ] WebSocket을 통한 실시간 업데이트
- [ ] 주차 공간 타입별 색상 구분
- [ ] 게임 시간 제한 기능

### API 연동 설정
현재 `parking_service.py`에서 실제 API를 사용하려면:
1. `self.api_base_url`을 실제 API 엔드포인트로 설정
2. `fetch_parking_data()` 메서드의 주석 처리된 API 호출 코드 활성화
3. 사용자가 제공한 데이터 구조에 맞는 API 응답 확인
